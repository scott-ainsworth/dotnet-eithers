...
namespace Results_IResult_Tests;

/// <summary>
///   Unit tests for <see cref="IResult{T}"/> constructors.
/// </summary>
[TestClass]
public class Constructor_Tests {

	/// <summary>
	///   The <see cref="IResult{T}"/> constructors' visibility is not public.
...
/// <summary>
///   Unit tests for <see cref="IEnumerable{T}.GetEnumerator"/>.
/// </summary>
[TestClass]
public class GetEnumerator_Tests {
	...
	///   <see cref="IEnumerator{T}"/> for a <see cref="IResult{T}"/> wrapping a value.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="IEnumerable{T}.GetEnumerator"/> methods return a correct
	///   <see cref="IEnumerator{T}"/> for a <see cref="IResult{T}"/> wrapping an error.
	/// </summary>
	[TestMethod]
	...
	/// </summary>
	[TestMethod]
	public void ResultT_IEnumerator_GetEnumerator_returns_correct_enumerator_for_wrapped_error() =>
		RunUnitTests(
			new IEnumerator_GetEnumerator_returns_correct_enumerator_for_wrapped_error());

	private sealed class IEnumerator_GetEnumerator_returns_correct_enumerator_for_wrapped_error
			: IUnitTest1 {
		public void RunTest<T>(T value) where T : notnull {
			var result = Result.From<T>(new ArgumentException("test"));
			var enumerator = ((IEnumerable)result).GetEnumerator();
...
/// <summary>
///   Unit tests for <see cref="IResult{T}.GetErrorEnumerator"/>.
/// </summary>
[TestClass]
public class GetErrorEnumerator_Tests {

	/// <summary>
	///   The <see cref="IResult{T}.GetErrorEnumerator"/> methods return an empty
	///   <see cref="IEnumerator{T}"/> for a <see cref="IResult{T}"/> wrapping a value.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="IResult{T}.GetErrorEnumerator"/> methods return a correct
	///   <see cref="IEnumerator{T}"/> for a <see cref="IResult{T}"/> wrapping an error.
	/// </summary>
	[TestMethod]
...
/// <summary>
///   Unit tests for <see cref="IResult{T}"/>.GetHashCode().
/// </summary>
[TestClass]
public class GetHashCode_Tests {
	...
	///   for <see cref="IResult{T}"/>s wrapping values.
	/// </summary>
	[TestMethod]
...
/// <summary>
///   Unit tests for <see cref="IResult{T}.IsValue"/>.
/// </summary>
[TestClass]
public class IsValue_and_IsError_Property_Tests {

	/// <summary>
	///   The <see cref="IResult{T}.IsError"/> property returns <see langword="false"/>
	///   for a <see cref="IResult{T}"/> wrapping a value.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="IResult{T}.IsError"/> property returns <see langword="true"/>
	///   for a <see cref="IResult{T}"/>s wrapping an error.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="IResult{T}.IsValue"/> property returns <see langword="false"/>
	///   for a <see cref="IResult{T}"/>s wrapping an error.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="IResult{T}.IsValue"/> property returns <see langword="true"/>
	///   for a <see cref="IResult{T}"/> wrapping a value.
	/// </summary>
	[TestMethod]
...
/// <summary>
///   Unit tests for <see cref="Ok{T}.ToValue"/>.
/// </summary>
[TestClass]
public class ToValue_Tests {

	/// <summary>
	///   The <see cref="Ok{T}.ToValue"/> method returns the value provided
	...
	/// <summary>
	///   The <see cref="Ok{T}.ToValue"/> method should only be implemented
	///   on <see cref="Ok{T}"/> (not on <see cref="Err{T}"/>).
	///   This is a design assumptions check.
	/// </summary>
	[TestMethod]
	...
	private sealed class ToValue_is_only_available_on_ValueResult : IUnitTest1 {
		...
		public void RunTest<T>(T value) where T : notnull {
			var toValueName = nameof(Ok<T>.ToValue);
			var resultMethods =
				typeof(IResult<T>).GetMethods(flags).Where(m => m.Name == toValueName);
			Assert.IsFalse(resultMethods.Any());
			var valueMethods =
				typeof(Ok<T>).GetMethods(flags).Where(m => m.Name == toValueName);
			Assert.IsTrue(valueMethods.Any());
			var errorMethods =
				typeof(Err<T>).GetMethods(flags).Where(m => m.Name == toValueName);
...
public class ToException_Tests {
	...
	/// <summary>
	///   The <see cref="Err{T}.ToException"/> method returns the error provided
	///   when the instance was created.
	/// </summary>
	[TestMethod]
	...
	/// <summary>
	///   The <see cref="Err{T}.ToException"/> method should only be implemented
	///   on <see cref="Err{T}"/> (not on <see cref="Ok{T}"/>).
	///   This is a design assumptions check.
	/// </summary>
	[TestMethod]
	...
	private sealed class ToError_is_only_available_on_ErrorResult : IUnitTest1 {
		...
		public void RunTest<T>(T value) where T : notnull {
			var resultMethods = GetToErrorMethods<T>(typeof(IResult<T>));
			Assert.IsFalse(resultMethods.Any());
			var valueMethods = GetToErrorMethods<T>(typeof(Ok<T>));
			Assert.IsFalse(valueMethods.Any());
			var errorMethods = GetToErrorMethods<T>(typeof(Err<T>));
		...
		private static IEnumerable<MethodInfo> GetToErrorMethods<T>(Type type) where T : notnull =>
			type.GetMethods(flags).Where(m => m.Name == nameof(Err<T>.ToException));
	}
}

/// <summary>
///   Unit tests for <see cref="IResult{T}.TryGetValue(out T)"/>.
/// </summary>
[TestClass]
public class TryGetValue_Tests {

	/// <summary>
	///   The <see cref="IResult{T}.TryGetValue(out T)"/> method returns false when no value
	///   is wrapped.
	/// </summary>
	[TestMethod]
	...
	private sealed class TryGetValue_returns_false_and_default_value_for_NoneT : IUnitTest0 {
		public void RunTest<T>() where T : notnull {
			var result = (IResult<T>)Result.From<T>(new ArgumentException("test"));
	...
	/// <summary>
	///   The <see cref="IResult{T}.TryGetValue(out T)"/> method returns true and the wrapped
...
/// <summary>
///   Unit tests for <see cref="IResult{T}.TryGetException(out Exception)"/>.
/// </summary>
[TestClass]
public class TryGetException_Tests {

	/// <summary>
	///   The <see cref="IResult{T}.TryGetException(out Exception)"/> method returns true and
	...
	private sealed class TryGetException_returns_true_and_correct_exception_for_ErrorResultT
		...
		public void RunTest<T>(T value) where T : notnull {
			var ex = new ArgumentException("test");
			var result = (IResult<T>)Result.From<T>(ex);
	...
	/// <summary>
	///   The <see cref="IResult{T}.TryGetException(out Exception)"/> method returns false and
