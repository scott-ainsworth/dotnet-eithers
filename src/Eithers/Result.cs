...
/// <summary>
///   Methods to create <see cref="Result{T}"/>s.
/// </summary>
public static class Result {
    ...
    /// <summary>
    ///   Create a new <see cref="Result{T}"/> from a specified value.
    /// </summary>
    /// <typeparam name="T">The type of the value wrapped by the returned
    ///   <see cref="Maybe{T}"/>.</typeparam>
    /// <param name="value">the value to wrap.</param>
    /// <returns>
    ///   A <see cref="ValueResult{T}"/> wrapping <paramref name="value"/> . 
    /// </returns>
    ...
    public static ValueResult<T> From<T>(T value) where T : notnull {
    ...
    /// <summary>
    ///   Create a new <see cref="Result{T}"/> from a specified exception.
    /// </summary>
    /// <typeparam name="T">The type of the value wrapped by the returned
    ///   <see cref="Maybe{T}"/>.</typeparam>
    /// <param name="ex">the <see cref="Exception"/> to wrap.</param>
    /// <returns>
    ///   An <see cref="ErrorResult{T}"/> wrapping <paramref name="ex"/> . 
    /// </returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="ex"/>
    ///   is <see langword="null"/>, which should not happen if null analysis
    ///   is enabled (<c>#nullable enable</c>).</exception>
    public static ErrorResult<T> From<T>(Exception ex) where T : notnull {
        // Null check for callers that don't use code analyzers to catch errors
...
#pragma warning disable CA1715
    ...
    /// <summary>
    ///   Creates a new <see cref="Result{T}"/> from the results of executing a function.
    /// </summary>
    /// <typeparam name="T">The type of the value wrapped by the returned
    ///   <see cref="Maybe{T}"/>.</typeparam>
    ...
    /// <returns>
    ///   A <see cref="Result{T}"/> representing the result of executing
    ///   <paramref name="func"/>.
    /// </returns>
    ...
    ///       wrapping the value is returned.</item>
    ///     <item><paramref name="func"/> throws an exception of type <typeparamref name="E1"/>
    ///       or <typeparamref name="E2"/>; An <see cref="ErrorResult{T}"/> wrapping the
    ///       exception is returned.</item>
    ///     <item><paramref name="func"/> throws any other exception; the exception is
    ///       re-thrown.</item>
    ///   </list>
    /// </remarks>
    public static Result<T> From<T, E1, E2>(Func<T> func)
    ...
    ///       <see cref="ErrorResult{T}"/> wrapping the exception is returned.</item>
    ///     <item><paramref name="func"/> throws any other exception; the exception is
    ///       re-thrown.</item>
    ///   </list>
    /// </remarks>
    public static Result<T> From<T, E1, E2, E3>(Func<T> func)
    ...
    ///       wrapping the value is returned.</item>
    ///     <item><paramref name="func"/> throws an exception of type <typeparamref name="E1"/> –
    ///     <typeparamref name="E4"/>; An <see cref="ErrorResult{T}"/> wrapping the exception
    ///       is returned.</item>
    ///     <item><paramref name="func"/> throws any other exception; the exception is
    ///       re-thrown.</item>
    ///   </list>
    /// </remarks>
    public static Result<T> From<T, E1, E2, E3, E4>(Func<T> func)
            where T : notnull where E1 : Exception where E2 : Exception where E3 : Exception
            where E4 : Exception {
        // Null check for callers that don't use code analyzers to catch errors
        _ = func ?? throw new ArgumentNullException(nameof(func));
    ...
    ///       wrapping the value is returned.</item>
    ///     <item><paramref name="func"/> throws an exception of type <typeparamref name="E1"/> –
    ///     <typeparamref name="E5"/>; An <see cref="ErrorResult{T}"/> wrapping the exception
    ///       is returned.</item>
    ///     <item><paramref name="func"/> throws any other exception; the exception is
    ///       re-thrown.</item>
    ///   </list>
    /// </remarks>
    public static Result<T> From<T, E1, E2, E3, E4, E5>(Func<T> func)
            where T : notnull where E1 : Exception where E2 : Exception where E3 : Exception
...
#pragma warning restore CA1715
    ...
    ///   is <see langword="null"/>, which should not happen if null analysis
    ///   is enabled (<c>#nullable enable</c>).</exception>
    public static Result<T> ToResult<T>(this Exception ex) where T : notnull {
        // Null check for callers that don't use code analyzers to catch errors
        _ = ex ?? throw new ArgumentNullException(nameof(ex));
        return new ErrorResult<T>(ex);
    }
